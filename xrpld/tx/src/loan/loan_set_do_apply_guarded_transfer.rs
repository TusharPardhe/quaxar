//! Current Rust helper mirroring the guarded-transfer
//! the LoanSet transactor shell after ledger state and property math are
//! already loaded.
//!
//! This module preserves the deterministic behavior around:
//!
//! - checking vault available first,
//! - then checking vault maximum,
//! - then running the representability loop,
//! - then running `checkLoanGuards(...)`,
//! - and only then entering the landed post-computed-values transfer shell,
...
use crate::{
    ...
    check_loan_set_do_apply_representability, check_loan_set_do_apply_vault_available,
    check_loan_set_do_apply_vault_maximum, run_loan_set_do_apply_check_loan_guards,
    run_loan_set_do_apply_post_computed_transfer,
};

#[allow(clippy::too_many_arguments)]
pub fn run_loan_set_do_apply_guarded_transfer<
    Tx,
    Asset,
    Amount,
    Properties,
...
{
    if let Err(err) = check_loan_set_do_apply_vault_available(vault_available, principal_requested)
    {
        return err.ter();
    }

    if let Err(err) =
        check_loan_set_do_apply_vault_maximum(vault_maximum, vault_total, interest_due, zero)
    {
        return err.ter();
    }
...
mod tests {
    ...
    use super::run_loan_set_do_apply_guarded_transfer;
    use crate::{
    ...
    #[test]
    fn loan_set_do_apply_guarded_transfer_uses_current_on_success() {
    ...
    #[test]
    fn loan_set_do_apply_guarded_transfer_returns_vault_funds_failure_first() {
        let steps = RefCell::new(Vec::new());
    ...
    #[test]
    fn loan_set_do_apply_guarded_transfer_returns_precision_loss_before_loan_guards() {
        let steps = RefCell::new(Vec::new());
    ...
    #[test]
    fn loan_set_do_apply_guarded_transfer_returns_loan_guard_failure_before_post_computed() {
        let steps = RefCell::new(Vec::new());
    ...
    #[test]
    fn loan_set_do_apply_guarded_transfer_returns_post_computed_failure_unchanged() {
        let result = run_loan_set_do_apply_guarded_transfer(
            &valid_tx(),
            &"USD",
            &100_i64,
