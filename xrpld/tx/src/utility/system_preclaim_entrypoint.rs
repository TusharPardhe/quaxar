//! Public system-family `preclaim(...)` composition shell above the landed
...
//!   types,
//! - and composes the landed system invoke-preclaim helper without re-encoding
...
use crate::system_transactor_metadata::run_with_system_transactor_txn_type_key;
use crate::{
    HasTxnType, PreclaimContext, PreclaimResult, PreflightResult, TxConsequences,
    UNKNOWN_TRANSACTION_TYPE_TER, txn_type_of,
};

pub fn run_system_preclaim_for_txn_type<Registry, View, Tx, Journal, ParentBatchId, E>(
...
pub fn run_system_preclaim_for_txn_source<Registry, View, Tx, Journal, ParentBatchId, E>(
    preflight_result: PreflightResult<Tx, TxConsequences, Journal, ParentBatchId>,
    registry: Registry,
    view: View,
    current_rules: &Rules,
    ...
    ParentBatchId: Clone,
{
    ...
    run_system_preclaim_for_txn_type(
        preflight_result,
        txn_type,
        registry,
        view,
