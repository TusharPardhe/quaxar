//! Lock-scope wrapper for the current `xrpld` `TxQ::apply(...)` boundary.
//!
...
//! - one owner object carries the landed apply state,
//! - callers still supply the live tx/runtime and ledger-view inputs,
...
use crate::{
    ...
    QueueApplyCurrentPreclaimClearRuntime, QueueApplyHoldPreflightTxSource,
    QueueApplyJournalLockScopeOwner, QueueApplyJournalOwner, QueueApplyLedgerViewSource,
...
pub trait QueueApplyLockScope {}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct QueueApplyLockScopeOwner<Account, Tx, Journal, ParentBatchId> {
...
{
    pub const fn new(owner: QueueApplyOwnerShell<Account, Tx, Journal, ParentBatchId>) -> Self {
        Self { owner }
    }
    ...
    pub fn into_journal_lock_scope_owner<Sink>(
        self,
        journal: Sink,
    ) -> QueueApplyJournalLockScopeOwner<Account, Tx, Journal, ParentBatchId, Sink> {
        QueueApplyJournalLockScopeOwner::new(QueueApplyJournalOwner::new(self.owner, journal))
    }
}
    ...
    ParentBatchId: Clone,
{
    fn with_lock_scope_owner<Lock, R>(
        &mut self,
        _lock: &mut Lock,
        run: impl FnOnce(&mut QueueApplyOwnerShell<Account, Tx, Journal, ParentBatchId>) -> R,
    ) -> R
    where
    ...
    where
        ...
        ApplySandbox: FnOnce(),
    {
        self.owner.apply_after_preflight(
            tx_source,
            view_source,
            preflight_result,
            hold_preflight,
    ...
    where
        ...
        TxSource: QueueApplyObservedTxSource<Account = Account, TransactionId = TxId>,
    {
        self.with_lock_scope_owner_current_app_view(lock, |owner| {
    ...
    where
        ...
        >,
    {
        self.with_lock_scope_owner_current_app_view_caller_preclaim(lock, |owner| {
    ...
    where
        ...
        InfoFn: FnMut(String),
    {
        self.with_lock_scope_owner_current_app_view_caller_preclaim_log_sinks(lock, |owner| {
    ...
    where
        ...
        InfoFn: FnMut(String),
    {
        self.with_lock_scope_owner_current_app_view_log_sinks(lock, |owner| {
        ...
        InfoFn: FnMut(String),
    {
        self.with_lock_scope_owner_current_app_view_log_sinks(lock, |owner| {
            owner.apply_with_app_view_and_owned_metrics_and_log_sinks_and_derived_preflight_facts(
                app, view, tx_source, trace, debug, info,
            )
        })
    }
