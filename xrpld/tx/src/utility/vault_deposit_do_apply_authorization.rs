//! Front the reference implementation load and authorization shell.
//!
//! This ports the exact current behavior around:
//!
//! - `tefINTERNAL` when the vault lookup is unexpectedly missing,
//! - `tefINTERNAL` when the share issuance lookup is unexpectedly missing,
//! - private non-owner deposits enforcing MPToken authorization first,
//! - public or owner deposits ensuring the share MPToken exists,
//! - private-owner deposits performing the additional issuer/holder
//!   authorization,
//! - and returning the first failing `TER` unchanged.

use protocol::{Ter, is_tes_success};

pub trait VaultDepositDoApplyAuthorizationVault {
    type Asset;
    type AccountId;
    type ShareId;
    type Amount;
...
pub trait VaultDepositDoApplyAuthorizationIssuance {
    type AccountId;
...
pub struct VaultDepositDoApplyAuthorizeRequest<'a, ShareId, AccountId, Balance> {
    pub prior_balance: &'a Balance,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct VaultDepositDoApplyAuthorizationState<Vault, Issuance, Asset, AccountId, ShareId> {
    pub vault: Vault,
...
pub fn load_vault_deposit_do_apply_authorization<
    Vault,
    Issuance,
    Balance,
    ReadVault,
...
>(
    ...
    VaultDepositDoApplyAuthorizationState<
        Vault,
        Issuance,
        Vault::Asset,
        Vault::AccountId,
...
>
where
    Vault: VaultDepositDoApplyAuthorizationVault,
    Vault::Asset: Clone,
    ...
    ) -> Ter,
{
    ...
    let share_issuance = read_issuance(&share_mpt_id).ok_or(Ter::TEF_INTERNAL)?;

    if vault.is_private() && submitter != vault.owner() {
        let ter = enforce_authorization(&share_mpt_id, submitter, prior_balance);
        if !is_tes_success(ter) {
            return Err(ter);
        }
    } else {
        if !mptoken_exists(&share_mpt_id, submitter) {
            let ter = authorize(VaultDepositDoApplyAuthorizeRequest {
                prior_balance,
                share_mpt_id: &share_mpt_id,
                account: submitter,
                holder: None,
            });
            if !is_tes_success(ter) {
        ...
        if vault.is_private() {
            let ter = authorize(VaultDepositDoApplyAuthorizeRequest {
                prior_balance,
                share_mpt_id: &share_mpt_id,
                account: share_issuance.issuer(),
                holder: Some(submitter),
            });
            if !is_tes_success(ter) {
                return Err(ter);
            }
...
mod tests {
    ...
    #[test]
    fn vault_deposit_do_apply_authorization_returns_tefinternal_when_issuance_is_missing() {
        let enforce_called = Cell::new(false);
