//! Integration tests that pin the higher loaded-owner `PaymentChannelFund.cpp`
...
#[test]
fn loaded_guarded_fund_apply_returns_no_entry_before_owner_lookup() {
    let owner_called = Cell::new(false);
    let mut sink = TestApplySink::new();

    let result = run_payment_channel_fund_loaded_guarded_do_apply(
...
#[test]
fn loaded_guarded_fund_apply_returns_owner_errors_before_destination() {
...
#[test]
fn loaded_guarded_fund_apply_preserves_success_order() {
    let mut sink = TestApplySink::new();
