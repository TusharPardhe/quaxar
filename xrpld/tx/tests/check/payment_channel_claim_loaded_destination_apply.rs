//! Integration tests that pin the destination-aware loaded claim seam to the
//! current C++ behavior.
...
use tx::payment_channel_claim_loaded_apply::run_payment_channel_claim_loaded_do_apply;
use tx::payment_channel_claim_loaded_destination_apply::{
    ...
    run_payment_channel_claim_loaded_destination_do_apply,
...
fn prepared_destination_apply_facts() -> PaymentChannelClaimLoadedPreparedDestinationApplyFacts<u32>
{
    build_payment_channel_claim_loaded_prepared_destination_apply_facts(
        loaded_channel(),
        loaded_tx(),
...
#[test]
fn loaded_claim_destination_apply_returns_no_target_before_sink_work() {
    let mut sink = TestApplySink::new();

    let result = run_payment_channel_claim_loaded_destination_do_apply(
        None,
        loaded_tx(),
        60_u32,
        true,
...
#[test]
fn loaded_claim_destination_apply_returns_no_dst_before_deposit_preauth() {
    let mut sink = TestApplySink::new();
...
#[test]
fn loaded_claim_destination_apply_matches_direct_claim_do_apply() {
    let mut helper_sink = TestApplySink::new();
    let helper_result = run_payment_channel_claim_loaded_destination_do_apply(
        Some(loaded_channel()),
        loaded_tx(),
        60_u32,
        true,
    ...
    let mut loaded_apply_sink = TestApplySink::new();
    let loaded_apply_result = run_payment_channel_claim_loaded_do_apply(
        Some(loaded_channel()),
        loaded_tx(),
        60_u32,
        &mut loaded_apply_sink,
    );
    ...
    assert_eq!(loaded_apply_result, direct_result);
    assert_eq!(helper_sink.events, direct_sink.events);
    assert_eq!(loaded_apply_sink.events, direct_sink.events);
    assert_eq!(
        helper_sink.events,
        [
    ...
    assert_eq!(loaded_apply_sink.expirations, vec![90]);
}

#[test]
fn prepared_loaded_claim_destination_apply_matches_direct_path() {
    let mut prepared_sink = TestApplySink::new();
    let prepared_result = run_payment_channel_claim_loaded_prepared_destination_carrier_do_apply(
        prepared_destination_apply_facts(),
        &mut prepared_sink,
    );

    let mut direct_sink = TestApplySink::new();
    let direct_result = run_payment_channel_claim_loaded_destination_do_apply(
        Some(loaded_channel()),
        loaded_tx(),
        60_u32,
        true,
    ...
    assert_eq!(prepared_sink.expirations, direct_sink.expirations);
}
