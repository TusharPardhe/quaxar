//! Integration tests that pin the narrowed Rust `CheckCash.cpp` shell to the
//! current C++ behavior.
...
use tx::{
    ...
    CheckCashIouLimitSide, CheckCashIouTrustCreateCallerPlan,
...
#[test]
fn check_cash_preclaim_rejects_missing_accounts() {
...
#[test]
fn check_cash_preclaim_rejects_path_partial_cases() {
...
#[test]
fn check_cash_preclaim_rejects_issuer_auth_cases() {
...
#[test]
fn check_cash_plan_iou_trust_create_uses_cpp_dest_low_and_limit_shape() {
    let create =
        run_check_cash_plan_iou_trust_create(30_u32, 20_u32, false, true, true, 0_u64, 0_u64)
            .unwrap();
    let existing =
        run_check_cash_plan_iou_trust_create(20_u32, 20_u32, true, false, false, 0_u64, 0_u64)
            .unwrap();

    assert_eq!(
        create,
        CheckCashIouTrustCreatePlan::<u64, u64> {
            needs_create: true,
            can_create: true,
            dest_low: true,
...
fn check_cash_plan_iou_trust_create_reports_cpp_reserve_gate() {
    let missing_reserve =
        run_check_cash_plan_iou_trust_create(30_u32, 20_u32, false, false, true, 0_u64, 0_u64);
...
#[test]
fn check_cash_plan_iou_trust_create_handoff_truster_and_update_state() {
    let created = run_check_cash_plan_iou_trust_create_handoff(
        20_u32, 10_u32, 20_u32, 0xfeed_u64, 7_u32, false, true, false, 0_u64, 0_u64,
    )
    .unwrap();
    let existing = run_check_cash_plan_iou_trust_create_handoff(
        30_u32, 10_u32, 20_u32, 0xbeef_u64, 9_u32, true, false, true, 0_u64, 0_u64,
    )
    .unwrap();

    assert_eq!(
        created,
        CheckCashIouTrustCreateHandoffPlan::<u32, u64, u32, u64, u64> {
            truster: 10,
            trustline_key: 0xfeed,
            trustline_index: 7,
...
#[test]
fn check_cash_plan_iou_trust_create_destination_state() {
    assert_eq!(
        run_check_cash_plan_iou_trust_create_destination_state(4, false),
...
#[test]
fn check_cash_plan_iou_trust_create_caller_packages_destination_state_and_update_callback() {
    let plan = run_check_cash_plan_iou_trust_create_caller(
        30_u32, 10_u32, 20_u32, 0x77_u64, 88_u32, false, true, false, 4, -1_i32, 0_i32,
    )
    .unwrap();

    assert_eq!(
        plan,
        CheckCashIouTrustCreateCallerPlan {
            destination_state: CheckCashIouTrustCreateDestinationState {
                owner_count: 4,
                default_ripple_enabled: false,
            },
            trust_create: CheckCashIouTrustCreateHandoffPlan {
                truster: 20,
                trustline_key: 0x77,
                trustline_index: 88,
                destination_update_after_create: true,
...
#[test]
fn check_cash_plan_iou_trust_create_caller_keeps_no_update_callback_when_line_exists() {
    let plan = run_check_cash_plan_iou_trust_create_caller(
        20_u32, 10_u32, 20_u32, 0x88_u64, 99_u32, true, false, true, 9, 0_u64, 0_u64,
    )
    .unwrap();
...
#[test]
fn check_cash_plan_iou_trust_create_handoff_reports_cpp_reserve_gate() {
    let missing_reserve = run_check_cash_plan_iou_trust_create_handoff(
        30_u32, 10_u32, 20_u32, 0x1234_u64, 4_u32, false, false, true, 0_u64, 0_u64,
    );
...
#[test]
fn check_cash_plan_iou_limit_override_preserves_limit_and_side_for_flow() {
    let low = run_check_cash_plan_iou_limit_override(99_u64, 10_u64, 1234_i64);
    let high = run_check_cash_plan_iou_limit_override(10_u64, 10_u64, 1234_i64);

    assert_eq!(
        low,
        CheckCashIouLimitOverridePlan {
            tweaked_limit_side: CheckCashIouLimitSide::Low,
            temporary_limit: 1234,
        }
    );
    assert_eq!(
        high,
...
#[test]
fn check_cash_build_iou_flow_deliver_uses_deliver_min_only_when_enabled() {
...
#[test]
fn check_cash_build_iou_flow_request_flow_packaging() {
    let amount_only = run_check_cash_build_iou_flow_request(55_u64, false, |value| value + 500);
    let deliver_min = run_check_cash_build_iou_flow_request(55_u64, true, |value| value + 500);
...
#[test]
fn check_cash_build_iou_flow_request_preserves_flow_packaging() {
    let request = run_check_cash_build_iou_flow_request(12_u32, false, |value| value + 8);
    let deliver_min_request =
        run_check_cash_build_iou_flow_request(12_u32, true, |value| value + 8);
...
#[test]
fn check_cash_finish_iou_flow_tracks_cpp_post_flow_gating() {
    let failure = run_check_cash_finish_iou_flow(
        CheckCashIouFlowResult {
            ter: Ter::TER_NO_RIPPLE,
            meets_requested_amount: true,
    ...
    let success_without_deliver_min = run_check_cash_finish_iou_flow(
        CheckCashIouFlowResult {
            ter: Ter::TES_SUCCESS,
            meets_requested_amount: true,
            meets_deliver_min: false,
...
fn check_cash_do_apply_iou_path_rejects_insufficient_reserve() {
    ...
    let result = run_check_cash_do_apply_iou_path(false, false, false, false, &mut sink);
...
#[test]
fn check_cash_do_apply_iou_path_keeps_flow_packaging_without_deliver_min() {
    let mut sink = TestApplySink::new();

    let result = run_check_cash_do_apply_iou_path(false, true, false, false, &mut sink);
...
#[test]
fn check_cash_do_apply_iou_path_restores_flow_limit_on_flow_failure() {
    let mut sink = TestApplySink::new();
...
#[test]
fn check_cash_do_apply_iou_path_returns_partial_when_deliver_min_is_not_satisfied() {
    let mut sink = TestApplySink::new();
    sink.run_iou_flow_result = CheckCashIouFlowResult {
        ter: Ter::TES_SUCCESS,
        meets_requested_amount: true,
...
#[test]
fn check_cash_do_apply_iou_path_skips_trust_create_when_line_exists() {
    let mut sink = TestApplySink::new();

    let result = run_check_cash_do_apply_iou_path(true, true, false, false, &mut sink);
...
#[test]
fn check_cash_do_apply_maps_missing_prechecked_state() {
    let mut sink = TestApplySink::new();
...
#[test]
fn check_cash_do_apply_returns_xrp_branch_failure_before_cleanup() {
    let mut sink = TestApplySink::new();
...
#[test]
fn check_cash_do_apply_iou_branch_stops_cleanup_after_destination_failure() {
    let mut sink = TestApplySink::new();
...
#[test]
fn check_cash_do_apply_maps_destination_dir_failure_before_owner_cleanup() {
    let mut sink = TestApplySink::new();
    sink.remove_destination_dir = false;
...
#[test]
fn check_cash_do_apply_maps_owner_dir_failure_before_tail_cleanup() {
    let mut sink = TestApplySink::new();
...
#[test]
fn check_cash_do_apply_uses_iou_branch_without_creating_line_when_already_present() {
    let mut sink = TestApplySink::new();
...
#[test]
fn check_cash_do_apply_delegates_iou_branch_and_preserves_cleanup_order() {
    let mut sink = TestApplySink::new();
...
#[test]
fn check_cash_do_apply_runs_xrp_success_path_in() {
    let mut sink = TestApplySink::new();
