//! Integration tests that pin the loaded preauth claim seam to the current
//! C++ behavior.
...
#[test]
fn loaded_claim_preauth_apply_returns_no_target_before_sink_work() {
    let mut sink = TestApplySink::new();
    let called = Rc::new(Cell::new(false));
    let called_for_closure = Rc::clone(&called);

    let result = run_payment_channel_claim_loaded_preauth_do_apply(
        None,
        loaded_tx(),
        60_u32,
        true,
...
#[test]
fn loaded_claim_preauth_apply_returns_no_dst_before_deposit_preauth() {
    let mut sink = TestApplySink::new();
    let called = Rc::new(Cell::new(false));
    let called_for_closure = Rc::clone(&called);
...
#[test]
fn loaded_claim_preauth_apply_stops_before_mutation_on_preauth_failure() {
    let mut sink = TestApplySink::new();
...
#[test]
fn loaded_claim_preauth_apply_matches_direct_claim_do_apply() {
    let mut helper_sink = TestApplySink::new();
    let helper_events = Rc::clone(&helper_sink.events);
    let helper_result = run_payment_channel_claim_loaded_preauth_do_apply(
        Some(loaded_channel()),
        loaded_tx(),
        60_u32,
        true,
