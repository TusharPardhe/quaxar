//! Integration tests that pin the destination lookup, preauth, and final
//! mutation loaded claim seam to the current C++ behavior.
...
use tx::payment_channel_claim_loaded_destination_preauth_mutation_apply::run_payment_channel_claim_loaded_destination_preauth_mutation_do_apply;
...
#[test]
fn loaded_claim_destination_preauth_mutation_returns_no_target_before_destination_lookup() {
    let destination_called = Rc::new(Cell::new(false));
    let destination_called_for_closure = Rc::clone(&destination_called);
    let preauth_called = Rc::new(Cell::new(false));
    let preauth_called_for_closure = Rc::clone(&preauth_called);
    let mutation_called = Rc::new(Cell::new(false));
    let mutation_called_for_closure = Rc::clone(&mutation_called);
    let mut sink = TestApplySink::new();

    let result = run_payment_channel_claim_loaded_destination_preauth_mutation_do_apply(
        None,
        loaded_tx(),
        60_u32,
        move || {
            destination_called_for_closure.set(true);
            true
        },
        move || {
            preauth_called_for_closure.set(true);
            Ter::TES_SUCCESS
        },
        move |_, _| {
            mutation_called_for_closure.set(true);
            Ter::TES_SUCCESS
        },
        &mut sink,
    );
    ...
    assert!(!preauth_called.get());
    assert!(!mutation_called.get());
    assert!(sink.events().is_empty());
}

#[test]
fn loaded_claim_destination_preauth_mutation_returns_no_dst_before_preauth() {
    let destination_called = Rc::new(Cell::new(false));
    let destination_called_for_closure = Rc::clone(&destination_called);
    let preauth_called = Rc::new(Cell::new(false));
    let preauth_called_for_closure = Rc::clone(&preauth_called);
...
#[test]
fn loaded_claim_destination_preauth_mutation_stops_before_mutation_on_preauth_failure() {
    let destination_called = Rc::new(Cell::new(false));
    let destination_called_for_closure = Rc::clone(&destination_called);
    let preauth_called = Rc::new(Cell::new(false));
    let preauth_called_for_closure = Rc::clone(&preauth_called);
...
#[test]
fn loaded_claim_destination_preauth_mutation_matches_existing_loaded_destination_preauth_seam() {
    let mut helper_sink = TestApplySink::new();
    let helper_events = Rc::clone(&helper_sink.events);
    let helper_result = run_payment_channel_claim_loaded_destination_preauth_mutation_do_apply(
        Some(loaded_channel()),
        loaded_tx(),
        60_u32,
        || true,
        ...
        |facts, sink| {
            let req_delta = facts.requested_balance_drops - facts.channel_balance_drops;
            sink.set_channel_balance(facts.requested_balance_drops);
            sink.add_destination_balance(req_delta);
