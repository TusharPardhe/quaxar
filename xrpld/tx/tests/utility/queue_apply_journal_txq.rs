...
use tx::{
    ...
    QueueApplyJournalLockScopeOwner, QueueApplyJournalSink, QueueApplyJournalTxQ,
    QueueApplyLiveOwner, QueueApplyLockScope, QueueApplyLockScopeOwner,
    ...
    QueueApplyPreparedPreclaimInputs, QueueApplyTxQ, QueueFeeMetricsSnapshot, QueueHoldPreflight,
    QueueViews, TxConsequences, TxQAccount, TxQSetup, format_direct_apply_finish_log_message,
...
fn build_full_queue_txq() -> QueueApplyTxQ<String, &'static str, &'static str, &'static str> {
    QueueApplyTxQ::new(QueueApplyLockScopeOwner::new(build_full_queue_owner_shell()))
}

fn build_direct_txq() -> QueueApplyTxQ<String, &'static str, &'static str, &'static str> {
    QueueApplyTxQ::new(QueueApplyLockScopeOwner::new(build_direct_owner_shell()))
}
...
#[test]
fn queue_apply_journal_txq_matches_journal_lock_scope_on_full_queue_path() {
    ...
    let mut txq = build_full_queue_txq().into_journal_txq(TestJournal::default());
    let mut lock = TestLockScope;
    let mut txq_app = queued_app();
    ...
    assert_eq!(
        txq.lock_scope_owner().owner().journal().emitted,
        vec![
            format!(
                "info:{}",
                format_queue_apply_full_queue_evict_info_message(
...
#[test]
fn queue_apply_journal_txq_log_messages_match_journal_lock_scope_on_full_queue_path() {
    let account = String::from("a");
    let tx_source = TestObservedTxSource {
        account: &account,
        transaction_id: "ABC123",
...
#[test]
fn queue_apply_journal_txq_matches_journal_lock_scope_after_preflight_on_direct_path() {
    let account = String::from("a");
    let tx_source = TestObservedTxSource {
        account: &account,
        transaction_id: "ABC123",
    ...
    let mut txq = build_direct_txq().into_journal_txq(TestJournal::default());
    let mut lock = TestLockScope;
    let mut txq_runtime = direct_app();
    ...
    assert_eq!(txq_runtime.preflight_calls, 0);
    assert_eq!(locked_runtime.direct_apply_calls, 1);
    assert_eq!(txq_runtime.direct_apply_calls, 1);
}

#[test]
fn queue_apply_journal_txq_after_preflight_log_messages_match_journal_lock_scope_on_direct_path() {
    let account = String::from("a");
    let tx_source = TestObservedTxSource {
        account: &account,
        transaction_id: "ABC123",
    ...
    assert_eq!(locked_runtime.direct_apply_calls, 1);
    assert_eq!(txq_runtime.direct_apply_calls, 1);
    assert_eq!(txq_runtime.trace_messages, locked_runtime.trace_messages);
}

#[test]
fn queue_apply_journal_txq_exposes_lock_scope_owner_accessors() {
    let mut txq = build_direct_txq().into_journal_txq(TestJournal::default());
...
#[test]
fn queue_apply_txq_into_journal_txq_matches_manual_wrapper_shape() {
    let promoted = build_direct_txq().into_journal_txq(TestJournal::default());
    let manual = QueueApplyJournalTxQ::new(QueueApplyJournalLockScopeOwner::new(
        build_direct_owner_shell().into_journal_owner(TestJournal::default()),
    ));

    assert_eq!(promoted.lock_scope_owner(), manual.lock_scope_owner());
}

#[test]
fn queue_apply_journal_txq_matches_journal_lock_scope_with_caller_preclaim_on_full_queue_path() {
    let account = String::from("a");
    let tx_source = TestObservedTxSource {
        account: &account,
        transaction_id: "ABC123",
...
#[test]
fn queue_apply_journal_txq_derives_hold_preflight_like_journal_lock_scope() {
    let account = String::from("a");
    let tx_source = TestObservedTxSource {
        account: &account,
        transaction_id: "ABC123",
...
#[test]
fn queue_apply_journal_txq_derives_preflight_facts_like_journal_lock_scope() {
    let account = String::from("a");
    let tx_source = TestObservedTxSource {
        account: &account,
        transaction_id: "ABC123",
...
#[test]
fn queue_apply_journal_txq_derives_preflight_and_hold_admission_like_journal_lock_scope() {
    let account = String::from("a");
    let tx_source = TestObservedTxSource {
        account: &account,
        transaction_id: "ABC123",
...
#[test]
fn queue_apply_journal_txq_after_preflight_derives_hold_preflight_like_journal_lock_scope() {
    let account = String::from("a");
    let tx_source = TestObservedTxSource {
        account: &account,
        transaction_id: "ABC123",
...
#[test]
fn queue_apply_journal_txq_after_preflight_derives_hold_admission_like_journal_lock_scope() {
    let account = String::from("a");
    let tx_source = TestObservedTxSource {
        account: &account,
        transaction_id: "ABC123",
...
#[test]
fn queue_apply_journal_txq_after_preflight_matches_journal_lock_scope_with_caller_preclaim_on_full_queue_path()
 {
    let account = String::from("a");
    let tx_source = TestObservedTxSource {
        account: &account,
...
#[test]
fn queue_apply_journal_txq_owned_metrics_match_journal_lock_scope_on_owner_snapshot_path() {
    let account = String::from("a");
    let tx_source = TestObservedTxSource {
        account: &account,
        transaction_id: "ABC123",
    ...
    let mut lock = TestLockScope;
    let mut txq_app = direct_app();
    let txq_stage = txq.apply_with_owned_metrics(
        &mut lock,
        &mut txq_app,
        &view,
        &tx_source,
...
#[test]
fn queue_apply_journal_txq_owned_metrics_derived_hold_preflight_match_journal_lock_scope_on_owner_snapshot_path()
 {
    let account = String::from("a");
    let tx_source = TestObservedTxSource {
        account: &account,
...
#[test]
fn queue_apply_journal_txq_owned_metrics_derived_preflight_facts_match_journal_lock_scope_on_owner_snapshot_path()
 {
    let account = String::from("a");
    let tx_source = TestObservedTxSource {
        account: &account,
...
#[test]
fn queue_apply_journal_txq_owned_metrics_derived_preflight_and_hold_admission_match_journal_lock_scope_on_owner_snapshot_path()
 {
    let account = String::from("a");
    let tx_source = TestObservedTxSource {
        account: &account,
...
#[test]
fn queue_apply_journal_txq_after_preflight_owned_metrics_derived_hold_preflight_match_journal_lock_scope_on_owner_snapshot_path()
 {
    let account = String::from("a");
    let tx_source = TestObservedTxSource {
        account: &account,
...
#[test]
fn queue_apply_journal_txq_after_preflight_owned_metrics_derived_hold_admission_match_journal_lock_scope_on_owner_snapshot_path()
 {
    let account = String::from("a");
    let tx_source = TestObservedTxSource {
        account: &account,
...
#[test]
fn queue_apply_journal_txq_after_preflight_owned_metrics_caller_preclaim_match_journal_lock_scope_on_owner_snapshot_path()
 {
    let account = String::from("a");
    let tx_source = TestObservedTxSource {
        account: &account,
    ...
    let mut lock = TestLockScope;
    let mut txq_app = direct_app();
    let txq_stage = txq.apply_after_preflight_with_owned_metrics_and_caller_preclaim(
        &mut lock,
        &mut txq_app,
        &view,
        &tx_source,
