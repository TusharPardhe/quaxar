...
//! `LoanSet.cpp::doApply()` loaded guarded-transfer shell to the current C++
//! behavior.
...
use tx::{
    ...
    LoanSetRepresentabilityField, run_loan_set_do_apply_loaded_guarded_transfer,
};
...
impl tx::LoanSetDoApplyLoadedPreGuardedTransferBroker for TestBroker {
...
#[test]
fn tx_loan_set_do_apply_loaded_guarded_transfer_uses_current_on_success() {
...
#[test]
fn tx_loan_set_do_apply_loaded_guarded_transfer_returns_limit_exceeded_from_loaded_debt_maximum() {
    let steps = Rc::new(RefCell::new(Vec::new()));
    let broker = TestBroker {
        management_fee_rate: 12,
        debt_total: 1_100,
...
#[test]
fn tx_loan_set_do_apply_loaded_guarded_transfer_returns_insufficient_funds_from_loaded_cover() {
    let steps = Rc::new(RefCell::new(Vec::new()));
    let broker = TestBroker {
        management_fee_rate: 12,
        debt_total: 1_000,
