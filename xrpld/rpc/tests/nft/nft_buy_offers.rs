...
#[test]
fn nft_buy_offers_rejects_sell_offer_marker_before_directory_scan() {
    let nft_id = Uint256::from_array([0xA1; 32]);
    let marker = offer_key(99);
    let root = nft_buy_offers_keylet(nft_id);
    let mut source = FakeSource {
        ledger: Some(closed_ledger()),
        ..Default::default()
    ...
    let mut sell_offer = make_offer(marker, nft_id, sample_account(0x11), 1, 0, false);
    sell_offer.set_field_u32(get_field_by_symbol("sfFlags"), 1);
    source
        .offers
        .insert(nft_offer_keylet(marker).key, sell_offer);

    let result = do_nft_buy_offers(
        &request(object([
            ("nft_id", JsonValue::String(nft_id.to_string())),
    ...
    assert_eq!(
        ...
        0,
        "a sell marker must be rejected before reading the buy-offer directory",
    );
}
