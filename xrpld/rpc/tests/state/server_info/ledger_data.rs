...
fn application_server_info_resolve_ledger_data_decrements_marker() {
    ...
    for i in 1..=5u8 {
        ...
        entries.push(sle);
        mutations.push((StateBatchOp::Insert, keylet.key, serializer.get_data().to_vec()));
    }
    
    ledger.apply_state_batch(&mutations).expect("batch should apply");
    
    let ledger = Arc::new(ledger);
    app.on_validated_ledger(Arc::clone(&ledger));
    
    let source = ApplicationServerInfo::new(rpc::OwnedApplicationServerInfo::from_application_root(&app));
