...
impl RpcRuntime for ApplicationRoot {
    ...
    fn peers_get(&self) -> JsonValue {
        self.overlay_runtime()
            .as_ref()
            .map(|o| {
                let peers: Vec<JsonValue> = o.overlay().active_peers_snapshot().iter().map(|p| {
                    p.json()
                }).collect();
...
mod tests {
    ...
    use super::{
        ...
        RpcSyncFilterStore, TM_QUERY_INDIRECT, can_send_primary_request,
        extend_acquisition_peer_set, get_ledger_query_type, select_missing_node_ids_for_request,
        send_get_ledger_to_peers, send_reply_mode_requests_to_peers, send_to_peers,
        set_get_ledger_query_depth, should_arm_timeout_local_check, should_run_local_check,
        target_made_progress, use_aggressive_by_hash, use_aggressive_by_hash_timeout,
        use_blind_request_trigger, use_reply_trigger, use_timeout_mode,
    };
    ...
    fn acquisition_peer_set_adds_best_remaining_peers() {
        ...
        let added = extend_acquisition_peer_set(
            &peers,
            LedgerRequestTarget::Hash(hash),
            0,
            &mut tracked,
            2,
        );
