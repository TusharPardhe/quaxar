...
mod tests {
    use super::{
        ...
        bind_server_runtime_into_root, bootstrap_acquire_budget_available,
        build_endpoint_broadcast, candidate_ledger_for_seq,
        candidate_reference_hash_from_reference_ledger, classify_completed_ledger_acceptance,
        classify_publish_advance, cold_bootstrap_persisted_validated_target,
        current_ledger_is_fresh, flush_nodestore_writes, hash_for_seq_from_reference_ledger,
        node_store_usage_path, path_size_bytes, peerfinder_canonical_ip,
...
"#,
    ...
    fn select_autoconnect_endpoints_prefers_low_hops_unique_ips_and_caps_attempts() {
        ...
        let selected =
            select_autoconnect_endpoints(&already_connected, &known, &HashMap::new(), now);
        assert!(
            !selected
                .iter()
    ...
    fn select_autoconnect_endpoints_skips_recent_attempted_ips_squelch() {
        ...
        let recent_attempts =
            HashMap::from([(first.ip(), now + PEERFINDER_RECENT_ATTEMPT_DURATION)]);
