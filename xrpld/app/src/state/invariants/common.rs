...
pub(super) fn account_holds_asset_amount<V: ApplyView + ?Sized>(
    ...
    match asset {
        Asset::Issue(issue) if issue.native() => Ok(Some(
            sandbox
        ...
        Asset::Issue(issue) => {
            if issue.issuer() == account {
                return Ok(Some(STAmount::from_iou_amount(
                    field,
                    IOUAmount::new(),
                    issue,
                )));
...
pub(super) fn account_holds_asset_number<V: ApplyView + ?Sized>(
    ...
    account: AccountID,
    asset: Asset,
) -> Result<Option<RuntimeNumber>, ledger::ViewError> {
    match asset {
        Asset::Issue(issue) if issue.native() => {
            let amount = sandbox
                ...
                .map(|sle| sle.get_field_amount(sf("sfBalance")))
                .unwrap_or_else(|| STAmount::from_xrp_amount(XRPAmount::new()));
            Ok(Some(amount_to_number(&amount)))
        }
        Asset::Issue(issue) => {
            if issue.issuer() == account {
                return Ok(Some(RuntimeNumber::zero()));
            }
            let amount = sandbox
                .read(protocol::line(account, issue.issuer(), issue.currency))?
                .map(|sle| {
