...
mod tests {
    ...
    use protocol::{
        AccountID, KeyType, LedgerEntryType, Rules, STAmount, STLedgerEntry, STTx, SecretKey,
        SeqProxy, Ter, TxType, account_keylet, calc_account_id, derive_public_key,
    ...
    fn application_root_applies_network_ops_pending_to_open_ledger_through_app_txq_runtime() {
        ...
        let mut parent =
            ledger_view_with_balance_and_owner_count(1, source, 1, 1_000_000_000, 0, &[]);
        parent.set_accepted(1_111, ledger::LEDGER_DEFAULT_TIME_RESOLUTION, true);
        let parent = Arc::new(parent);
        app.on_closed_ledger(Arc::clone(&parent));
        let _ = app.open_ledger().modify(|view| {
    ...
    fn application_root_submit_batch_reuses_live_ticket_and_sequence_state() {
        ...
        assert!(runtime.stage_transaction(
            Arc::clone(&sequence_payment_shared),
            false,
            false,
            false
        ));
    ...
    fn application_root_submit_batch_reuses_live_account_txn_id_state() {
        ...
        let (_, second) = signed_payment_tx_with_account_txn_id(
            0x31,
            destination,
            2,
    ...
    fn application_root_can_own_network_ops_runtime_and_bridge_held_tx_queue() {
        ...
        let outcome = app
            .apply_held_transactions_to_network_ops(
                SHAMapHash::new(Uint256::from_u64(101)),
                |sync| {
                    syncs
                        .lock()
