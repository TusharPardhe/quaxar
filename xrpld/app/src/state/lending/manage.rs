...
use protocol::{
    Asset, STLedgerEntry, STTx, Ter, feature_id, tfLoanDefault, tfLoanImpair, tfLoanUnimpair,
};

use super::common::*;
use super::helpers::*;

pub(super) fn unimpair_loan<V: ApplyView>(
    view: &mut V,
    loan_sle: &Arc<STLedgerEntry>,
    vault_sle: &Arc<STLedgerEntry>,
    vault_asset: Asset,
) -> Ter {
    let scale = vault_scale(vault_sle, vault_asset);
    let loss_reversed = loan_vault_exposure(vault_sle, loan_sle);
    let current_loss = vault_sle.get_field_number(sf("sfLossUnrealized")).value();
    if current_loss < loss_reversed {
        return Ter::TEF_BAD_LEDGER;
    }
    let updated_loss = adjust_imprecise_number(current_loss, -loss_reversed, vault_asset, scale);
...
pub fn apply_loan_manage<V: ApplyView>(view: &mut V, sttx: &STTx) -> Ter {
    ...
    let vault_sle = match view.peek(protocol::vault_keylet_from_key(vault_id)) {
        Ok(Some(sle)) => sle,
    ...
    let result = if tx_requests_default {
        ...
        let total_default_amount = loan_vault_exposure(&vault_sle, &loan_sle);
        ...
        if vault_total_assets < vault_default_amount {
            ...
            if std::env::var_os("QUAXAR_PARITY_NUMERIC_DIAGNOSTICS").is_some() {
                eprintln!(
                    "PARITY_LOAN_MANAGE scale={scale} totalDefault={total_default_amount} covered={default_covered} vaultDefault={vault_default_amount} rounded={vault_default_rounded} totalBefore={vault_total_assets} availableBefore={}",
            ...
            if vault_available_after > vault_total_after && !vault_asset.integral() {
                ...
                // Number::exponent() in rippled is the normalized internal
                // exponent.  `external_parts()` may shift it by one while
                // adapting a large internal mantissa to the public i64 form,
                // which changes the strict > 13 dust boundary.
                let available_exponent = vault_available_after.exponent;
                let difference_exponent = difference.exponent;
        ...
        let scale = vault_scale(&vault_sle, vault_asset);
        let loss_unrealized = loan_vault_exposure(&vault_sle, &loan_sle);
        let current_loss = vault_sle.get_field_number(sf("sfLossUnrealized")).value();
        let updated_loss =
        ...
        unimpair_loan(view, &loan_sle, &vault_sle, vault_asset)
    } else {
        Ter::TES_SUCCESS
    };
    ...
    if result == Ter::TES_SUCCESS && view.rules().enabled(&feature_id("fixCleanup3_1_3")) {
        ...
        if view.update(Arc::new(loan_update)).is_err() {
            return Ter::TEF_BAD_LEDGER;
        }
        ...
        if view.update(Arc::new(broker_update)).is_err() {
            return Ter::TEF_BAD_LEDGER;
        }
        ...
        if view.update(Arc::new(vault_update)).is_err() {
            return Ter::TEF_BAD_LEDGER;
        }
    }
