...
mod tests {
    ...
    fn app_rcl_consensus_bows_out_when_validator_list_is_expired() {
        ...
        assert!(
            journal
                ...
                .iter()
                .any(|(level, message)| level == "error"
                    && message.contains("expired validator list"))
        );
    }
    ...
    fn app_rcl_consensus_acquires_and_re_shares_real_sync_txsets() {
        ...
        {
            let _ = inbound.lock().expect("inbound tx mutex").give_set(
                txset_id,
                Arc::clone(&set),
                false,
            );
    ...
    fn app_rcl_consensus_make_txset_builds_sync_tree_and_relays_cached_disputed_tx() {
        ...
        let prev = consensus_ledger_from_ledger(&Ledger::from_ledger_seq_and_close_time(
            700, 1_200, false,
        ));
