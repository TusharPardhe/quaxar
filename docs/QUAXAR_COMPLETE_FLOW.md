...
flowchart TD
    ...
    %% ════════════════════════════════════════════════════════════════
    %% VALIDATION TRIE: TMValidation → add → updateTrie → getPreferred
    %% ════════════════════════════════════════════════════════════════

    FINAL --> VAL_WIRE[TMValidation arrives\noverlay message router]
    VAL_WIRE --> VAL_DESER[Deserialize STValidation\nis_current + trusted + suppression + sig check]
    VAL_DESER --> VAL_GATE{passes all gates?}
    VAL_GATE -->|No| VAL_DROP[Drop/charge]
    VAL_GATE -->|Yes| VAL_HANDLE[handle_new_validation\ndriver.rs:88-112 + rcl_validations.rs:347-414]
    VAL_HANDLE --> VAL_ADD[*** validations.add node_id, val ***\nvalidations.rs:550]

    %% ═══ add internals ═══
    VAL_ADD --> VADD_CUR{is_current?}
    VADD_CUR -->|No| VADD_STALE[return Stale]
    VADD_CUR -->|Yes| VADD_SEQ{seq_enforcers advancing?}
    VADD_SEQ -->|No| VADD_BAD[return BadSeq/Conflicting]
    VADD_SEQ -->|Yes| VADD_MAP{current.get node_id?}
    VADD_MAP -->|None + trusted| VUT_NEW[update_trie_validation\nval, None :611]
    VADD_MAP -->|Existing + newer + trusted| VUT_REPL[update_trie_validation\nval, Some old_key :618-620]
    VADD_MAP -->|Not trusted or stale| VADD_DONE[return Current]

    %% ═══ updateTrie: acquire gate ═══
    VUT_NEW --> VUT_BODY[Clear prior + check_acquired flush]
    VUT_REPL --> VUT_BODY
    VUT_BODY --> VUT_ACQ[*** adaptor.acquire ledger_id ***\nrcl_validation.rs:262]
    VUT_ACQ --> VACQ_LOCAL{self.ledgers.get?}
    VACQ_LOCAL -->|Hit| VTRIE[*** trie.insert ledger ***\nTRIE ADVANCES]
    VACQ_LOCAL -->|Miss| VACQ_HIST{ledger_history cache?}
    VACQ_HIST -->|Hit| VTRIE
    VACQ_HIST -->|Miss| VACQ_DEFER[acquiring_.insert\nAsync GetConsL2 fetch]

    %% ═══ Why caught-up node finds it: THE FIX ═══
    ACCEPT -.->|record_consensus_built_ledger\nregister_ledger inserts into\nself.ledgers HashMap\ncommit 61cf458| VACQ_LOCAL

    %% ═══ getPreferred: trie → preferred LCL ═══
    VTRIE -.->|Trie current| PREFERRED
    PREFERRED --> VGP[trie.get_preferred\nvalidations.rs:727]
    VGP --> VGP_HAVE{trie returned preferred?}
    VGP_HAVE -->|No| VGP_FALL[acquiring_ majority fallback]
    VGP_HAVE -->|Yes| VGP_PARENT{preferred.seq == curr.seq+1\nAND ancestor == curr.id?\n:742}
    VGP_PARENT -->|Yes: parent of next tip| VGP_STAY[return curr = LCL\nNO DEMOTION]
    VGP_PARENT -->|No| VGP_AHEAD{ahead or different chain?}
    VGP_AHEAD -->|Yes| VGP_SWITCH[return preferred → switch]
    VGP_AHEAD -->|No| VGP_STAY
    VGP_STAY -.-> SAME
    VGP_SWITCH -.-> SAME
    VGP_FALL -.-> SAME

    %% ═══ checkAccept from add ═══
    VUT_NEW --> VADD_DONE
    VUT_REPL --> VADD_DONE
    VADD_DONE --> VADD_OUT{Current + trusted?}
    VADD_OUT -->|Yes| VCHECK[check_accept hash, seq]
    VADD_OUT -->|No| VADD_RELAY[Relay]
    VCHECK -.-> ADVANCE
```
