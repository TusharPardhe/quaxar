...
impl SyncTree {
            ...
            FnMut(Vec<DeferredFetchRequestInfo>) -> Vec<Option<SharedIntrusive<SHAMapTreeNode>>>,
    {
        ...
        while !scan.is_complete() && scan.remaining() > 0 {
            // rippled `SHAMap::getMissingNodes` admits at most the declared
            // deferred-read bound before it waits for callbacks and resumes.
            // Do not use the legacy compatibility wrapper here: it preserves
            // an historical one-extra-read behavior that is not valid for the
            // direct asynchronous NodeStore descent path.
            scan.run_with_family_bounded(
                family,
                filter,
                max_deferred,
