    ...
    fn insert_recursive(
        ...
        match node.as_ref() {
            CowNode::Inner { children } => {
                let branch =
                    (key.data()[depth / 2] >> (if depth.is_multiple_of(2) { 4 } else { 0 })) & 0x0F;
            ...
            CowNode::Leaf {
                ...
                if existing_key == key {
                    ...
                    let eb = (existing_key.data()[depth / 2]
                        >> (if depth.is_multiple_of(2) { 4 } else { 0 }))
                        & 0x0F;
                    let nb = (key.data()[depth / 2]
                        >> (if depth.is_multiple_of(2) { 4 } else { 0 }))
                        & 0x0F;
                    if eb == nb {
                        // Same branch at this depth — recurse deeper.
                        let sub = Self::insert_recursive(
                            &Arc::new(CowNode::Leaf {
                                key: *existing_key,
                        ...
                        let mut children: [Option<Arc<CowNode>>; 16] = Default::default();
                        children[eb as usize] = Some(Arc::new(CowNode::Leaf {
                            key: *existing_key,
                            item: existing_item.clone(),
                        }));
