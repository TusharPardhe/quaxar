...
impl LedgerStateFix {
    pub const TX_TYPE: crate::TxType = crate::TxType::from_u16(53);

    pub fn new(tx: Arc<crate::STTx>) -> Result<Self, String> {
        if tx.get_txn_type() != Self::TX_TYPE {
            return Err("Invalid transaction type for LedgerStateFix".to_owned());
        }
        Ok(Self {
    ...
    pub fn get_ledger_fix_type(&self) -> u16 {
        self.base
            .as_sttx()
            .get_field_u16(crate::get_field_by_symbol("sfLedgerFixType"))
    }
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LedgerStateFixBuilder {
    base: crate::TransactionBuilderBase,
}

impl LedgerStateFixBuilder {
    pub fn new(
        ...
        let mut builder = Self {
            base: crate::TransactionBuilderBase::new(
                LedgerStateFix::TX_TYPE,
                account,
                sequence,
                fee,
            ),
        };
        builder = builder.set_ledger_fix_type(ledger_fix_type);
        builder
    }

    pub fn from_tx(tx: Arc<crate::STTx>) -> Result<Self, String> {
        if tx.get_txn_type() != LedgerStateFix::TX_TYPE {
            return Err("Invalid transaction type for LedgerStateFixBuilder".to_owned());
        }
        Ok(Self {
    ...
    pub fn set_ledger_fix_type(mut self, value: u16) -> Self {
        self.base
            .object_mut()
            .set_field_u16(crate::get_field_by_symbol("sfLedgerFixType"), value);
        self
    }
    ...
    ) -> Result<LedgerStateFix, crate::SignError> {
        self.base.sign(public_key, secret_key)?;
        Ok(LedgerStateFix::new(Arc::new(crate::STTx::from_stobject(
