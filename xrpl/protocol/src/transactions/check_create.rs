...
impl CheckCreate {
    pub const TX_TYPE: crate::TxType = crate::TxType::from_u16(16);

    pub fn new(tx: Arc<crate::STTx>) -> Result<Self, String> {
        if tx.get_txn_type() != Self::TX_TYPE {
            return Err("Invalid transaction type for CheckCreate".to_owned());
        }
        Ok(Self {
...
impl CheckCreateBuilder {
    pub fn new(
        ...
        let mut builder = Self {
            base: crate::TransactionBuilderBase::new(CheckCreate::TX_TYPE, account, sequence, fee),
        };
    ...
    pub fn from_tx(tx: Arc<crate::STTx>) -> Result<Self, String> {
        if tx.get_txn_type() != CheckCreate::TX_TYPE {
            return Err("Invalid transaction type for CheckCreateBuilder".to_owned());
        }
        Ok(Self {
    ...
    ) -> Result<CheckCreate, crate::SignError> {
        self.base.sign(public_key, secret_key)?;
        Ok(CheckCreate::new(Arc::new(crate::STTx::from_stobject(
