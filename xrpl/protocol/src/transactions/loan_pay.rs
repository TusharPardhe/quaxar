...
impl LoanPay {
    pub const TX_TYPE: crate::TxType = crate::TxType::from_u16(84);

    pub fn new(tx: Arc<crate::STTx>) -> Result<Self, String> {
        if tx.get_txn_type() != Self::TX_TYPE {
            return Err("Invalid transaction type for LoanPay".to_owned());
        }
        Ok(Self {
...
impl LoanPayBuilder {
    pub fn new(
        ...
        let mut builder = Self {
            base: crate::TransactionBuilderBase::new(LoanPay::TX_TYPE, account, sequence, fee),
        };
    ...
    pub fn from_tx(tx: Arc<crate::STTx>) -> Result<Self, String> {
        if tx.get_txn_type() != LoanPay::TX_TYPE {
            return Err("Invalid transaction type for LoanPayBuilder".to_owned());
        }
        Ok(Self {
    ...
    ) -> Result<LoanPay, crate::SignError> {
        self.base.sign(public_key, secret_key)?;
        Ok(LoanPay::new(Arc::new(crate::STTx::from_stobject(
