...
impl DIDSet {
    pub const TX_TYPE: crate::TxType = crate::TxType::from_u16(49);

    pub fn new(tx: Arc<crate::STTx>) -> Result<Self, String> {
        if tx.get_txn_type() != Self::TX_TYPE {
            return Err("Invalid transaction type for DIDSet".to_owned());
        }
        Ok(Self {
...
impl DIDSetBuilder {
    pub fn new(
        ...
        Self {
            base: crate::TransactionBuilderBase::new(DIDSet::TX_TYPE, account, sequence, fee),
        }
    }

    pub fn from_tx(tx: Arc<crate::STTx>) -> Result<Self, String> {
        if tx.get_txn_type() != DIDSet::TX_TYPE {
            return Err("Invalid transaction type for DIDSetBuilder".to_owned());
        }
        Ok(Self {
    ...
    ) -> Result<DIDSet, crate::SignError> {
        self.base.sign(public_key, secret_key)?;
        Ok(DIDSet::new(Arc::new(crate::STTx::from_stobject(
