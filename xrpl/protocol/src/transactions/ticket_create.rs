...
impl TicketCreate {
    pub const TX_TYPE: crate::TxType = crate::TxType::from_u16(10);

    pub fn new(tx: Arc<crate::STTx>) -> Result<Self, String> {
        if tx.get_txn_type() != Self::TX_TYPE {
            return Err("Invalid transaction type for TicketCreate".to_owned());
        }
        Ok(Self {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct TicketCreateBuilder {
...
impl TicketCreateBuilder {
    pub fn new(
        ...
        let mut builder = Self {
            base: crate::TransactionBuilderBase::new(TicketCreate::TX_TYPE, account, sequence, fee),
        };
        builder = builder.set_ticket_count(ticket_count);
        builder
    }

    pub fn from_tx(tx: Arc<crate::STTx>) -> Result<Self, String> {
        if tx.get_txn_type() != TicketCreate::TX_TYPE {
            return Err("Invalid transaction type for TicketCreateBuilder".to_owned());
        }
        Ok(Self {
    ...
    ) -> Result<TicketCreate, crate::SignError> {
        self.base.sign(public_key, secret_key)?;
        Ok(TicketCreate::new(Arc::new(crate::STTx::from_stobject(
