...
impl OfferCancel {
    pub const TX_TYPE: crate::TxType = crate::TxType::from_u16(8);

    pub fn new(tx: Arc<crate::STTx>) -> Result<Self, String> {
        if tx.get_txn_type() != Self::TX_TYPE {
            return Err("Invalid transaction type for OfferCancel".to_owned());
        }
        Ok(Self {
...
impl OfferCancelBuilder {
    pub fn new(
        ...
        let mut builder = Self {
            base: crate::TransactionBuilderBase::new(OfferCancel::TX_TYPE, account, sequence, fee),
        };
    ...
    pub fn from_tx(tx: Arc<crate::STTx>) -> Result<Self, String> {
        if tx.get_txn_type() != OfferCancel::TX_TYPE {
            return Err("Invalid transaction type for OfferCancelBuilder".to_owned());
        }
        Ok(Self {
    ...
    ) -> Result<OfferCancel, crate::SignError> {
        self.base.sign(public_key, secret_key)?;
        Ok(OfferCancel::new(Arc::new(crate::STTx::from_stobject(
