...
impl Clawback {
    pub const TX_TYPE: crate::TxType = crate::TxType::from_u16(30);

    pub fn new(tx: Arc<crate::STTx>) -> Result<Self, String> {
        if tx.get_txn_type() != Self::TX_TYPE {
            return Err("Invalid transaction type for Clawback".to_owned());
        }
        Ok(Self {
...
impl ClawbackBuilder {
    pub fn new(
        ...
        let mut builder = Self {
            base: crate::TransactionBuilderBase::new(Clawback::TX_TYPE, account, sequence, fee),
        };
    ...
    pub fn from_tx(tx: Arc<crate::STTx>) -> Result<Self, String> {
        if tx.get_txn_type() != Clawback::TX_TYPE {
            return Err("Invalid transaction type for ClawbackBuilder".to_owned());
        }
        Ok(Self {
    ...
    ) -> Result<Clawback, crate::SignError> {
        self.base.sign(public_key, secret_key)?;
        Ok(Clawback::new(Arc::new(crate::STTx::from_stobject(
