...
impl VaultDeposit {
    pub const TX_TYPE: crate::TxType = crate::TxType::from_u16(68);

    pub fn new(tx: Arc<crate::STTx>) -> Result<Self, String> {
        if tx.get_txn_type() != Self::TX_TYPE {
            return Err("Invalid transaction type for VaultDeposit".to_owned());
        }
        Ok(Self {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct VaultDepositBuilder {
    base: crate::TransactionBuilderBase,
}

impl VaultDepositBuilder {
    pub fn new(
        ...
        let mut builder = Self {
            base: crate::TransactionBuilderBase::new(VaultDeposit::TX_TYPE, account, sequence, fee),
        };
    ...
    pub fn from_tx(tx: Arc<crate::STTx>) -> Result<Self, String> {
        if tx.get_txn_type() != VaultDeposit::TX_TYPE {
            return Err("Invalid transaction type for VaultDepositBuilder".to_owned());
        }
        Ok(Self {
    ...
    ) -> Result<VaultDeposit, crate::SignError> {
        self.base.sign(public_key, secret_key)?;
        Ok(VaultDeposit::new(Arc::new(crate::STTx::from_stobject(
