...
impl XChainAddClaimAttestation {
    pub const TX_TYPE: crate::TxType = crate::TxType::from_u16(45);

    #[allow(clippy::too_many_arguments)]
    pub fn new(tx: Arc<crate::STTx>) -> Result<Self, String> {
        if tx.get_txn_type() != Self::TX_TYPE {
            return Err("Invalid transaction type for XChainAddClaimAttestation".to_owned());
        }
        Ok(Self {
...
impl Deref for XChainAddClaimAttestation {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct XChainAddClaimAttestationBuilder {
...
impl XChainAddClaimAttestationBuilder {
    ...
    pub fn new(
        ...
        let mut builder = Self {
            base: crate::TransactionBuilderBase::new(
                XChainAddClaimAttestation::TX_TYPE,
                account,
                sequence,
                fee,
            ),
    ...
    pub fn from_tx(tx: Arc<crate::STTx>) -> Result<Self, String> {
        if tx.get_txn_type() != XChainAddClaimAttestation::TX_TYPE {
            return Err("Invalid transaction type for XChainAddClaimAttestationBuilder".to_owned());
        }
        Ok(Self {
    ...
    ) -> Result<XChainAddClaimAttestation, crate::SignError> {
        self.base.sign(public_key, secret_key)?;
        Ok(
            XChainAddClaimAttestation::new(Arc::new(crate::STTx::from_stobject(
