...
impl OracleDelete {
    pub const TX_TYPE: crate::TxType = crate::TxType::from_u16(52);

    pub fn new(tx: Arc<crate::STTx>) -> Result<Self, String> {
        if tx.get_txn_type() != Self::TX_TYPE {
            return Err("Invalid transaction type for OracleDelete".to_owned());
        }
        Ok(Self {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct OracleDeleteBuilder {
...
impl OracleDeleteBuilder {
    pub fn new(
        ...
        let mut builder = Self {
            base: crate::TransactionBuilderBase::new(OracleDelete::TX_TYPE, account, sequence, fee),
        };
    ...
    pub fn from_tx(tx: Arc<crate::STTx>) -> Result<Self, String> {
        if tx.get_txn_type() != OracleDelete::TX_TYPE {
            return Err("Invalid transaction type for OracleDeleteBuilder".to_owned());
        }
        Ok(Self {
    ...
    ) -> Result<OracleDelete, crate::SignError> {
        self.base.sign(public_key, secret_key)?;
        Ok(OracleDelete::new(Arc::new(crate::STTx::from_stobject(
