...
impl CredentialAccept {
    pub const TX_TYPE: crate::TxType = crate::TxType::from_u16(59);

    pub fn new(tx: Arc<crate::STTx>) -> Result<Self, String> {
        if tx.get_txn_type() != Self::TX_TYPE {
            return Err("Invalid transaction type for CredentialAccept".to_owned());
        }
        Ok(Self {
...
impl Deref for CredentialAccept {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct CredentialAcceptBuilder {
...
impl CredentialAcceptBuilder {
    ...
    pub fn from_tx(tx: Arc<crate::STTx>) -> Result<Self, String> {
        if tx.get_txn_type() != CredentialAccept::TX_TYPE {
            return Err("Invalid transaction type for CredentialAcceptBuilder".to_owned());
        }
        Ok(Self {
    ...
    ) -> Result<CredentialAccept, crate::SignError> {
        self.base.sign(public_key, secret_key)?;
        Ok(CredentialAccept::new(Arc::new(crate::STTx::from_stobject(
