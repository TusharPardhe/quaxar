...
impl PermissionedDomainDelete {
    pub const TX_TYPE: crate::TxType = crate::TxType::from_u16(63);

    pub fn new(tx: Arc<crate::STTx>) -> Result<Self, String> {
        if tx.get_txn_type() != Self::TX_TYPE {
            return Err("Invalid transaction type for PermissionedDomainDelete".to_owned());
        }
        Ok(Self {
    ...
    pub fn get_domain_id(&self) -> basics::base_uint::Uint256 {
        self.base
            .as_sttx()
...
impl Deref for PermissionedDomainDelete {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PermissionedDomainDeleteBuilder {
    base: crate::TransactionBuilderBase,
}

impl PermissionedDomainDeleteBuilder {
    pub fn new(
        ...
        let mut builder = Self {
            base: crate::TransactionBuilderBase::new(
                PermissionedDomainDelete::TX_TYPE,
                account,
                sequence,
                fee,
            ),
        };
        builder = builder.set_domain_id(domain_id);
        builder
    }

    pub fn from_tx(tx: Arc<crate::STTx>) -> Result<Self, String> {
        if tx.get_txn_type() != PermissionedDomainDelete::TX_TYPE {
            return Err("Invalid transaction type for PermissionedDomainDeleteBuilder".to_owned());
        }
        Ok(Self {
    ...
    ) -> Result<PermissionedDomainDelete, crate::SignError> {
        self.base.sign(public_key, secret_key)?;
        Ok(
            PermissionedDomainDelete::new(Arc::new(crate::STTx::from_stobject(
