...
impl MPTokenAuthorize {
    pub const TX_TYPE: crate::TxType = crate::TxType::from_u16(57);

    pub fn new(tx: Arc<crate::STTx>) -> Result<Self, String> {
        if tx.get_txn_type() != Self::TX_TYPE {
            return Err("Invalid transaction type for MPTokenAuthorize".to_owned());
        }
        Ok(Self {
...
impl Deref for MPTokenAuthorize {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct MPTokenAuthorizeBuilder {
...
impl MPTokenAuthorizeBuilder {
    ...
    pub fn from_tx(tx: Arc<crate::STTx>) -> Result<Self, String> {
        if tx.get_txn_type() != MPTokenAuthorize::TX_TYPE {
            return Err("Invalid transaction type for MPTokenAuthorizeBuilder".to_owned());
        }
        Ok(Self {
    ...
    ) -> Result<MPTokenAuthorize, crate::SignError> {
        self.base.sign(public_key, secret_key)?;
        Ok(MPTokenAuthorize::new(Arc::new(crate::STTx::from_stobject(
