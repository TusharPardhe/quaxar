...
impl MPTokenIssuanceDestroy {
    pub const TX_TYPE: crate::TxType = crate::TxType::from_u16(55);

    pub fn new(tx: Arc<crate::STTx>) -> Result<Self, String> {
        if tx.get_txn_type() != Self::TX_TYPE {
            return Err("Invalid transaction type for MPTokenIssuanceDestroy".to_owned());
        }
        Ok(Self {
...
impl Deref for MPTokenIssuanceDestroy {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct MPTokenIssuanceDestroyBuilder {
...
impl MPTokenIssuanceDestroyBuilder {
    pub fn new(
        ...
        let mut builder = Self {
            base: crate::TransactionBuilderBase::new(
                MPTokenIssuanceDestroy::TX_TYPE,
                account,
                sequence,
                fee,
            ),
    ...
    pub fn from_tx(tx: Arc<crate::STTx>) -> Result<Self, String> {
        if tx.get_txn_type() != MPTokenIssuanceDestroy::TX_TYPE {
            return Err("Invalid transaction type for MPTokenIssuanceDestroyBuilder".to_owned());
        }
        Ok(Self {
    ...
    ) -> Result<MPTokenIssuanceDestroy, crate::SignError> {
        self.base.sign(public_key, secret_key)?;
        Ok(
            MPTokenIssuanceDestroy::new(Arc::new(crate::STTx::from_stobject(
