...
impl VaultDelete {
    pub const TX_TYPE: crate::TxType = crate::TxType::from_u16(67);

    pub fn new(tx: Arc<crate::STTx>) -> Result<Self, String> {
        if tx.get_txn_type() != Self::TX_TYPE {
            return Err("Invalid transaction type for VaultDelete".to_owned());
        }
        Ok(Self {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct VaultDeleteBuilder {
    base: crate::TransactionBuilderBase,
}

impl VaultDeleteBuilder {
    pub fn new(
        ...
        let mut builder = Self {
            base: crate::TransactionBuilderBase::new(VaultDelete::TX_TYPE, account, sequence, fee),
        };
    ...
    pub fn from_tx(tx: Arc<crate::STTx>) -> Result<Self, String> {
        if tx.get_txn_type() != VaultDelete::TX_TYPE {
            return Err("Invalid transaction type for VaultDeleteBuilder".to_owned());
        }
        Ok(Self {
    ...
    ) -> Result<VaultDelete, crate::SignError> {
        self.base.sign(public_key, secret_key)?;
        Ok(VaultDelete::new(Arc::new(crate::STTx::from_stobject(
