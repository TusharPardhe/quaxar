...
impl NFTokenBurn {
    pub const TX_TYPE: crate::TxType = crate::TxType::from_u16(26);

    pub fn new(tx: Arc<crate::STTx>) -> Result<Self, String> {
        if tx.get_txn_type() != Self::TX_TYPE {
            return Err("Invalid transaction type for NFTokenBurn".to_owned());
        }
        Ok(Self {
...
impl NFTokenBurnBuilder {
    pub fn new(
        ...
        let mut builder = Self {
            base: crate::TransactionBuilderBase::new(NFTokenBurn::TX_TYPE, account, sequence, fee),
        };
    ...
    pub fn from_tx(tx: Arc<crate::STTx>) -> Result<Self, String> {
        if tx.get_txn_type() != NFTokenBurn::TX_TYPE {
            return Err("Invalid transaction type for NFTokenBurnBuilder".to_owned());
        }
        Ok(Self {
    ...
    ) -> Result<NFTokenBurn, crate::SignError> {
        self.base.sign(public_key, secret_key)?;
        Ok(NFTokenBurn::new(Arc::new(crate::STTx::from_stobject(
