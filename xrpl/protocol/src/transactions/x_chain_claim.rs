...
impl XChainClaim {
    pub const TX_TYPE: crate::TxType = crate::TxType::from_u16(43);

    pub fn new(tx: Arc<crate::STTx>) -> Result<Self, String> {
        if tx.get_txn_type() != Self::TX_TYPE {
            return Err("Invalid transaction type for XChainClaim".to_owned());
        }
        Ok(Self {
...
impl XChainClaimBuilder {
    pub fn new(
        ...
        let mut builder = Self {
            base: crate::TransactionBuilderBase::new(XChainClaim::TX_TYPE, account, sequence, fee),
        };
    ...
    pub fn from_tx(tx: Arc<crate::STTx>) -> Result<Self, String> {
        if tx.get_txn_type() != XChainClaim::TX_TYPE {
            return Err("Invalid transaction type for XChainClaimBuilder".to_owned());
        }
        Ok(Self {
    ...
    ) -> Result<XChainClaim, crate::SignError> {
        self.base.sign(public_key, secret_key)?;
        Ok(XChainClaim::new(Arc::new(crate::STTx::from_stobject(
