...
/// Returns the complete pinned `transactions.macro` dispatch catalog in
/// canonical registry order.
///
/// Consumers should compare against this iterator rather than maintaining a
/// second manual count, so a newly registered transaction cannot silently
/// bypass an exhaustive dispatch audit.
pub fn dispatchable_tx_types() -> impl ExactSizeIterator<Item = TxType> + Clone {
    DISPATCHABLE_TX_TYPES
        .iter()
        .map(|(value, _, _)| TxType::from_u16(*value))
}
